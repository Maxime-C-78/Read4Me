...
class App():
    ...
    """
    def __init__(self, keypad_mapping):
        ...
        self.keypad_mapping = keypad_mapping  # Dictionnaire des touches → actions
    ...
    def wait(self):
        ...
        keyboard.wait() # Attend un événement clavier
            
    def setup_keyboard_listener(self):
        """Configure l'écoute des touches du clavier"""
        for action, key in self.keypad_mapping.items():
            keyboard.on_press_key(key, lambda event, action=action: self.handle_key(action))

    def handle_key(self, action):
        """Exécute l'action correspondant à la touche appuyée"""
        print(f"Touche détectée → Action : {action}")
        callback = self.callbacks[action.value]  # Récupère la fonction associée
        callback()  # Exécute l'action
